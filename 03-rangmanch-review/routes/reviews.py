...
def list_reviews(
    ...
    reviews = session.exec(query).all()  # all() Returns all matching results as a list.
    return reviews
...
def get_average_rating(play_name: str, session: Session = Depends(get_session)):
    ...
    ).first()  # first() gets the first result from the query.
    
    avg_rating, total_reviews = result  # bcz result = (4.0, 3)
...
def update_review(review_id: int, update: ReviewUpdate, session: Session = Depends(get_session)):
    ...
    for key, value in update_data.items():
        setattr(review, key, value)  # setattr() is a Python built-in function used to set/update an attribute of an object.
